...
public class EspecialidadDAO implements Serializable{
	...
	public Especialidad leerEspecialidad(String espNombre) {
		
		String sql = "SELECT esp FROM Especialidad esp where esp.espnombreEspecialidad=:nombre";
		Query query = em.createQuery(sql, Especialidad.class);
		query.setParameter("nombre", espNombre);
		Especialidad especialidad = (Especialidad) query.getSingleResult();
